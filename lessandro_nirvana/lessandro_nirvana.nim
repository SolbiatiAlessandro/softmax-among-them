...
when not defined(italkalotLibrary):
  import bitworld/ais/openrouter, whisky
...
when not defined(italkalotLibrary):
  ...
  proc votingChatMessages(
    ...
    frame: string,
    turn: int
  ): seq[openrouter.ConversationMessage] =
    ## Builds OpenAI messages for one voting chat turn.
    @[
      openrouter.ConversationMessage(
        role: "system",
        content:
  ...
  proc startOpenAiRequest(
    bot: var Bot,
    kind: AiRequestKind,
    messages: openArray[openrouter.ConversationMessage],
    turn = 0
  ): bool =
    ## Starts one pollable OpenAI request for the bot.
    if openrouter.aiKey.len == 0 or bot.aiRequestKind != AiNoRequest:
      return false
    ...
    bot.aiRequestTag =
      $bot.aiRequestSerial & "-" & kind.aiRequestLabel() &
        "-" & $bot.frameTick
    result = openrouter.startTalkToAI(messages, bot.aiRequestTag)
    if not result:
      bot.clearAiRequest()
  ...
  proc pollOpenAiRequest(bot: var Bot): bool =
    ...
    let response = openrouter.pollTalkToAI()
...
when isMainModule and not defined(italkalotLibrary):
  ...
  echo "starting lessandro_nirvana -> ", target
