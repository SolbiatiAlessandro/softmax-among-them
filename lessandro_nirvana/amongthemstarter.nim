...
type
  ...
  Bot = object
    ...
    # Nirvana improvements:
    taskRouteOrder: seq[int]   # nearest-neighbor TSP ordering of task indices
    taskRouteBuilt: bool       # true once the route is computed
    imposterFakePauseTicks: int  # ticks remaining in fake-task pause
    imposterKillTargetX: int   # world X of crewmate we're hunting
    imposterKillTargetY: int   # world Y of crewmate we're hunting
    imposterHuntingKill: bool  # true when actively approaching a target
...
proc resetRoundState(bot: var Bot) =
  ...
  # Nirvana: reset route and kill-hunt state between rounds
  bot.taskRouteOrder.setLen(0)
  bot.taskRouteBuilt = false
  bot.imposterFakePauseTicks = 0
  bot.imposterKillTargetX = 0
  bot.imposterKillTargetY = 0
  bot.imposterHuntingKill = false
...
proc readVoteChatLines(bot: Bot, count: int): seq[VoteChatLine] =
...
proc randomVoteDelay(bot: var Bot): int =
  ...
  VoteListenBaseTicks + bot.rng.rand(VoteListenRandomTicks - 1)
...
proc bodyRoomMessage(bot: Bot, x, y: int): string =
  ...
  let suspect = bot.bodySuspectedColor()
  result =
    if room == "unknown":
      "body"
    else:
...
proc desiredVotingTarget(bot: Bot): int =
  ...
  if bodyTarget != VoteUnknown:
    return bodyTarget
  let seenTarget = bot.seenVotingTargetFrom(bot.lastSeenTicks)
  if seenTarget != VoteUnknown:
    return seenTarget
  bot.votePlayerCount
...
proc decideVotingMask(bot: var Bot): uint8 =
  ...
  if not instantVote and listenedTicks < bot.voteDelayTicks:
    ...
    bot.intent = "ready, listening in vote chat " &
      $listenedTicks & "/" & $bot.voteDelayTicks
    bot.thought(bot.intent)
    return 0
  bot.desiredMask =
...
proc buildTaskRoute(bot: var Bot) =
  ## Nirvana: builds a nearest-neighbor TSP task route to minimize travel.
  ## Called once per round on first goal selection; uses heuristic distance
  ## from task center to task center for speed (no A* required here).
  let n = bot.sim.tasks.len
  if n == 0:
    bot.taskRouteBuilt = true
    return
  bot.taskRouteOrder.setLen(0)
  var visited = newSeq[bool](n)
  # Start from the task nearest to the player's current world position.
  var currentX = bot.playerWorldX()
  var currentY = bot.playerWorldY()
  for _ in 0 ..< n:
    var bestIdx = -1
    ...
    for j in 0 ..< n:
      if visited[j]:
        continue
      let center = bot.sim.tasks[j].taskCenter()
      let d = heuristic(currentX, currentY, center.x, center.y)
      if d < bestDist:
        bestDist = d
        bestIdx = j
    if bestIdx < 0:
    ...
    bot.taskRouteOrder.add(bestIdx)
    let chosen = bot.sim.tasks[bestIdx].taskCenter()
    currentX = chosen.x
    currentY = chosen.y
  bot.taskRouteBuilt = true

proc routeOrderedTaskGoal(
  bot: Bot,
  state: TaskState,
  checkStates: bool,
  checkCheckout: bool,
  ...
  ## Nirvana: picks the next task in nearest-neighbor TSP route order.
  ## Falls back to nearest if route order produces nothing.
  for routeIdx in bot.taskRouteOrder:
    if checkStates and bot.taskStates.len == bot.sim.tasks.len:
      let s = bot.taskStates[routeIdx]
      if state == TaskMandatory and s != TaskMandatory:
        continue
      if s == TaskCompleted:
        continue
    if checkCheckout and (bot.checkoutTasks.len != bot.sim.tasks.len or
        not bot.checkoutTasks[routeIdx]):
      continue
    if checkRadar and (bot.radarTasks.len != bot.sim.tasks.len or
        not bot.radarTasks[routeIdx]):
      continue
    let goal = bot.taskGoalFor(routeIdx, state)
    if goal.found:
      return goal
...
proc nearestTaskGoal(
  ...
  ## Nirvana improved: uses nearest-neighbor TSP route order to reduce travel.
  ## Visible task icons (mandatory) are still prioritised by screen proximity
  ## so we can grab opportunistic tasks when walking past them.
  if not bot.taskRouteBuilt:
    bot.buildTaskRoute()

  # 1. Visible task icon right here on screen — always grab the nearest one.
  var bestDistance = high(int)
  for i in 0 ..< bot.sim.tasks.len:
  ...
  # 2. Already committed to a mandatory task — stick with it.
  ...
  # 3. Next mandatory task in route order.
  result = bot.routeOrderedTaskGoal(TaskMandatory, true, false, false)
  if result.found:
    return

  # 4. Already committed to a checkout task — stick with it.
  if bot.goalIndex >= 0 and
      bot.goalIndex < bot.sim.tasks.len and
      bot.taskStates.len == bot.sim.tasks.len and
  ...
  # 5. Next checkout task in route order.
  result = bot.routeOrderedTaskGoal(TaskMaybe, false, true, false)
  if result.found:
    return

  # 6. Already committed to a radar task — stick with it.
  if bot.goalIndex >= 0 and
      bot.goalIndex < bot.sim.tasks.len and
  ...
  # 7. Next radar task in route order.
  result = bot.routeOrderedTaskGoal(TaskMaybe, false, false, true)
  if result.found:
    return
...
proc decideImposterMask(bot: var Bot): uint8 =
  ## Nirvana improved: smarter kill timing + fake-task pause deception.
  bot.radarDots.setLen(0)
  if bot.radarTasks.len != bot.sim.tasks.len:
    bot.radarTasks = newSeq[bool](bot.sim.tasks.len)
  if bot.checkoutTasks.len != bot.sim.tasks.len:
  ...
  # Flee from any visible body (same as before).
  ...
  # Nirvana improvement 2: kill-ready hunting logic.
  # When the kill button is lit, prioritise approaching any crewmate.
  # We only EXECUTE the kill when that crewmate is lone (no witnesses).
  # This lets us stalk while the cooldown charges, then strike immediately.
  if bot.imposterKillReady:
    let loneCrewmate = bot.loneVisibleCrewmate()
    if loneCrewmate.found:
      # Lone target — close or pursue.
      ...
      if bot.inKillRange(target.x, target.y):
        ...
        bot.intent = "nirvana kill lone crewmate"
        bot.desiredMask = ButtonA
        bot.controllerMask = ButtonA
        bot.hasPathStep = false
        bot.path.setLen(0)
        bot.thought("lone crewmate in range, nirvana strike")
        return ButtonA
      # Chase the lone target.
      bot.imposterHuntingKill = true
      bot.imposterKillTargetX = target.x
      bot.imposterKillTargetY = target.y
      bot.goalIndex = -2
      return bot.navigateToPoint(
        target.x,
        target.y,
      ...
      # Multiple crewmates visible — back off hunting flag, wait for opening.
      # But continue toward the same fake-task area so we don't look suspicious
      # by chasing someone while others are watching.
      bot.imposterHuntingKill = false
  else:
    # Kill on cooldown — reset any active hunt.
    bot.imposterHuntingKill = false

  # Nirvana improvement 3: fake-task pause simulation.
  # When we arrive at a fake-task spot, pause for a realistic number of ticks
  # before moving on, so we look like we're completing a task.
  if bot.imposterFakePauseTicks > 0:
    dec bot.imposterFakePauseTicks
    bot.intent = "imposter fake task pause=" & $bot.imposterFakePauseTicks
    bot.desiredMask = 0
    bot.controllerMask = 0
    bot.hasPathStep = false
    bot.path.setLen(0)
    bot.thought("nirvana fake-task pause at " & bot.goalName)
    return 0

  # Choose or maintain the current fake target.
  ...
  if not goal.found:
    ...
    return 0
  # Arrived at fake target — start a pause that mimics task completion.
  if heuristic(bot.playerWorldX(), bot.playerWorldY(), goal.x, goal.y) <=
      TaskPreciseApproachRadius:
    # Pause for roughly half a real task's ticks, then pick the next target.
    bot.imposterFakePauseTicks = bot.sim.config.taskCompleteTicks div 2
    bot.imposterGoalIndex = bot.randomFakeTargetIndex()
    goal = bot.fakeTargetGoalFor(bot.imposterGoalIndex)
    if not goal.found:
...
proc initBot(mapPath = ""): Bot =
  ...
  # Nirvana: initialise extra state fields
  result.taskRouteBuilt = false
  result.imposterFakePauseTicks = 0
  result.imposterKillTargetX = 0
  result.imposterKillTargetY = 0
  result.imposterHuntingKill = false
...
when isMainModule and not defined(italkalotLibrary):
  ...
  echo "starting amongthemstarter -> ", target
